 ...
 * Base signal = cosine of pseudo-embeddings; pronouns get a heuristic boost
 * toward earlier tokens (coreference intuition). Returns an n×n row-stochastic
 * matrix where row i = how much token i attends to each token j.
 */
export function attentionMatrix(tokens: string[]): number[][] {
  const embs = tokens.map(embed);
  return tokens.map((tok, i) => {
    const scores = tokens.map((_, j) => {
      let s = cosine(embs[i], embs[j]) * 4;
      if (i === j) s += 1.2; // mild self-attention
      if (isPronoun(tok) && j < i) s += 1.6 * (1 - (i - j) / tokens.length); // prefer nearer earlier words
      return s;
    });
